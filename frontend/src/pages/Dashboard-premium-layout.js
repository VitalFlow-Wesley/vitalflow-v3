...
const fallbackInsights = [
  [CheckCircle2, "Excelente recuperação", "Seu corpo está se recuperando muito bem. Continue assim!", "text-emerald-300"],
  [Star, "Consistência é chave", "7 dias seguidos monitorando. Mantenha o ritmo!", "text-amber-300"],
  [Droplets, "Atenção à hidratação", "Sua ingestão de água está um pouco abaixo do ideal.", "text-violet-300"],
  [Target, "Próximo objetivo", "Mantenha 7+ horas de sono para otimizar ainda mais.", "text-rose-300"],
];
...
function formatHour(timestamp, fallbackIndex = 0) {
  ...
  if (Number.isNaN(date.getTime())) return `${String(fallbackIndex * 2).padStart(2, "0")}:00`;
...
function deriveStatus(score) {
  if (score >= 80) return { label: "Normal", tone: "text-emerald-300", badge: "bg-emerald-400/10 text-emerald-300", text: "Equilíbrio preservado" };
  if (score >= 60) return { label: "Atenção", tone: "text-amber-300", badge: "bg-amber-400/10 text-amber-300", text: "Ajuste recomendado" };
  return { label: "Crítico", tone: "text-rose-300", badge: "bg-rose-400/10 text-rose-300", text: "Intervenção sugerida" };
}
...
function deriveRecommendation(score, trend, predictiveAlert) {
  if (predictiveAlert?.has_alert && predictiveAlert?.alert?.message) {
    return {
      ...
      reason: "A IA identificou um padrão de risco e está antecipando uma ação preventiva antes da sobrecarga.",
      duration: "5 min",
      type: "Preventiva",
      focus: "Prevenção",
      priority: "Média",
  ...
  if (score < 60) {
    return {
      ...
      description: "Faça uma pausa guiada curta para reduzir carga fisiológica e recuperar estabilidade.",
      reason: "Seu V-Score está abaixo da zona ideal. A recomendação prioriza recuperação rápida antes de novas demandas.",
      duration: "7 min",
      type: "Recuperação",
  ...
  if (trend === "falling") {
    return {
      ...
      reason: "Existe tendência de queda nos últimos registros, então a IA recomenda preservar energia e evitar sobrecarga.",
      duration: "4 min",
      type: "Manutenção",
  ...
  return {
    ...
    reason: "Seu V-Score está estável, HRV preservada e stress controlado. Este é o momento ideal para manter consistência.",
    duration: "3 min",
    type: "Manutenção",
    focus: "Manutenção leve",
    priority: "Baixa",
...
function PremiumCard({ children, className = "" }) {
  return (
    <div
      className={`rounded-[22px] border border-white/[0.07] bg-[#0b0d0f] shadow-[0_24px_70px_rgba(0,0,0,0.35),inset_0_1px_0_rgba(255,255,255,0.03)] ${className}`}
    >
      {children}
    </div>
  );
...
function SectionLabel({ children }) {
  return (
    <div className="text-[11px] font-bold uppercase tracking-[0.32em] text-slate-400/80">
      {children}
    </div>
  );
}
...
export default function Dashboard() {
  ...
  const quick = [
    [Activity, "Estável nas últimas 6h", healthTrend?.trend === "falling" ? "Tendência de queda" : "Variação mínima detectada", healthTrend?.trend === "falling" ? "text-amber-300" : "text-emerald-300"],
    [Brain, currentScore >= 80 ? "Stress controlado" : "Stress em atenção", currentScore >= 80 ? "Carga mental dentro do ideal" : "Reduza estímulos por alguns minutos", currentScore >= 80 ? "text-amber-300" : "text-rose-300"],
    [HeartPulse, "HRV preservada", "Bom sinal de recuperação", "text-cyan-300"],
    [ShieldCheck, recommendation.priority === "Alta" ? "Janela de recuperação" : "Boa janela para manutenção", recommendation.priority === "Alta" ? "Priorize pausa curta" : "Aproveite para manter consistência", "text-emerald-300"],
  ];

  const executiveSummary = [
    ["Prioridade", recommendation.priority, recommendation.priority === "Alta" ? "Ação recomendada" : "Momento favorável", Sprout, recommendation.priority === "Alta" ? "text-rose-300" : "text-emerald-300"],
  ...
  const insights = healthTrend?.requires_intervention
    ? [
        [AlertTriangle, "Atenção necessária", healthTrend.intervention_message || "Seus indicadores pedem cuidado preventivo.", "text-rose-300"],
