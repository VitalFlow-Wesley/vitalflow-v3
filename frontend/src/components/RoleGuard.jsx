...
export default function RoleGuard({ children, minRole="Gestor", redirectTo="/" }) {
  ...
  if ((ROLE_LEVELS[user.nivel_acesso]??99) > (ROLE_LEVELS[minRole]??99)) return <Navigate to={redirectTo} replace />;
  return children;
}
export function ShowIfRole({ minRole, children }) {
  const { user } = useAuth();
  if (!user) return null;
  if ((ROLE_LEVELS[user.nivel_acesso]??99) > (ROLE_LEVELS[minRole]??99)) return null;
  return children;
}
