...
source = source
  .replace(
    ...
    'className="pointer-events-none absolute z-10 min-w-[96px] rounded-lg border border-cyan-300/20 bg-[#071115]/95 px-2 py-1.5 text-[10px] shadow-[0_10px_22px_rgba(0,0,0,0.30)] backdrop-blur"'
  )
  .replace(
    /top: Math\.max\(12, activePoint\.y \* 1\.95 - 32\)/g,
    "top: Math.max(10, activePoint.y * 1.95 - 24)"
  )
  .replace(
    /className="text-\[10px\] font-bold uppercase tracking-\[0\.22em\] text-slate-400"/g,
    'className="text-[9px] font-bold uppercase tracking-[0.16em] text-slate-400"'
  )
  .replace(
  ...
  .replace(
    ...
    'className="pb-0.5 text-[9px] text-slate-400"'
  )
  .replace(
