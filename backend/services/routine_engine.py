...
def build_smart_routine_payload(current_analysis: Dict[str, Any], history: List[Dict[str, Any]] = None) -> Dict[str, Any]:
    history = history or []

    input_data = current_analysis.get("input_data", {}) or {}
    real_data = current_analysis.get("real_data", {}) or {}

    stress = _safe_float(current_analysis.get("stress") or current_analysis.get("stress_score"))
    recovery = _safe_float(current_analysis.get("recovery") or current_analysis.get("recovery_score"), 100)
    ...
    sleep_hours = _safe_float(input_data.get("sleep_hours") or real_data.get("sleep_hours"))
    ...
    breathing_critical = _make_routine(
        ...
        steps=[
            ...
            {"label": "Expire", "seconds": 6, "instruction": "Solte o ar devagar pela boca"},
            {"label": "Segure", "seconds": 2, "instruction": "Pause antes do próximo ciclo"},
        ],
        how_it_works=[
    ...
    breathing_balance = _make_routine(
        ...
        status="atencao",
        steps=[
            {"label": "Inspire", "seconds": 4, "instruction": "Inspire pelo nariz"},
            {"label": "Segure", "seconds": 4, "instruction": "Segure o ar"},
            {"label": "Expire", "seconds": 4, "instruction": "Expire devagar"},
    ...
    breathing_maintenance = _make_routine(
        ...
        status="normal",
        steps=[
            {"label": "Inspire", "seconds": 3, "instruction": "Inspire suavemente"},
            {"label": "Segure", "seconds": 3, "instruction": "Segure com leveza"},
            {"label": "Expire", "seconds": 3, "instruction": "Expire sem pressa"},
            {"label": "Segure", "seconds": 3, "instruction": "Prepare o próximo ciclo"},
        ],
        how_it_works=[
    ...
    if status == "critico":
        if has_high_stress:
            ...
            explanation = "Seu estado indica sobrecarga imediata. A prioridade é baixar a ativação fisiológica."
        elif has_sleep_deficit or has_low_recovery:
            primary = walk_reset
            alternatives = [hydration_boost, breathing_critical]
            explanation = "Seu corpo mostra desgaste acumulado. Movimento leve pode ajudar a reativar seu sistema sem sobrecarregar."
        else:
            primary = breathing_critical
            alternatives = [hydration_boost, mental_pause]
            explanation = "Seu estado exige uma intervenção curta e direta para recuperar estabilidade."
    elif status == "atencao":
        if has_screen_fatigue:
            ...
            primary = walk_reset
            alternatives = [hydration_boost, breathing_balance]
            explanation = "Há sinais de baixa ativação corporal. Uma caminhada leve pode ajudar agora."
        elif has_sleep_deficit:
            primary = hydration_boost
            alternatives = [breathing_balance, visual_reset]
            explanation = "Seu estado pede uma intervenção leve para recuperar energia e clareza."
        else:
            primary = breathing_balance
            alternatives = [visual_reset, mental_pause]
            explanation = "Uma rotina curta de equilíbrio pode ajudar a recuperar seu estado."
    else:
        if has_good_state:
            primary = breathing_maintenance
            alternatives = [mental_pause, hydration_boost]
            explanation = "Seu estado está estável. O objetivo agora é manter consistência e prevenir queda."
        else:
            primary = mental_pause
            alternatives = [hydration_boost, breathing_maintenance]
            explanation = "Seu estado está controlado, mas uma pausa curta pode melhorar foco e manutenção."
