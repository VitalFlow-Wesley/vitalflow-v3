...
async def get_team_overview(request: Request, period: str = "7d", setor: str = ""):
    try:
        ...
        colab_ids = None
        if setor:
            colabs = await db.colaboradores.find(
                colab_filter,
                {"_id": 0, "id": 1}
            ).to_list(10000)
        ...
        trend_7d = [
            {
                "date": d,
                "avg_v_score": round(sum(s) / len(s), 1),
                "total_analyses": len(s),
            }
...
async def export_dashboard_pdf(request: Request, period: str = "7d"):
    try:
        ...
        elements.append(
            Paragraph(
                f"Periodo: Ultimos {period_label} | Gerado em {now.strftime('%d/%m/%Y as %H:%M')} | Por: {colaborador['nome']}",
                subtitle_style,
            )
        )
        ...
        if burnout_risk > 0:
            elements.append(
                Paragraph(
                    f"ATENCAO: {burnout_risk} colaborador(es) com V-Score medio abaixo de 50 no periodo. A legislacao exige intervencao preventiva.",
                    body_style,
                )
            )
        else:
            elements.append(
                Paragraph(
                    "Nenhum colaborador em risco critico no periodo analisado. Empresa em conformidade.",
                    body_style,
                )
            )
        ...
        elements.append(
            Paragraph(
                "Dados 100% anonimizados conforme LGPD. Nenhum colaborador individual e identificado neste relatorio.",
                ParagraphStyle(
                    "Footer",
                    parent=styles["Normal"],
                    fontSize=8,
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=vitalflow_relatorio_{period}_{now.strftime('%Y%m%d')}.pdf"
            },
        )
    except HTTPException:
        raise
