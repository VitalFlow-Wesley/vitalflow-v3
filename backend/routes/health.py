...
async def export_personal_pdf(request: Request, period: str = "7d"):
    try:
        ...
        if account_type == "personal":
            ...
            if not is_premium and str(colaborador.get("email", "")).lower() not in allowed_emails:
                raise HTTPException(status_code=403, detail="Recurso exclusivo do Plano Premium.")
            if premium_expires_at:
                try:
                    exp = datetime.froimport io
import logging
from datetime import datetime, timezone, timedelta
...
async def export_personal_pdf(request: Request, period: str = "7d"):
    try:
        ...
        # Libera exportação para contas premium e para as contas do proprietário.
        allowed_emails = {
        ...
        colaborador_email = str(
            ...
            or colaborador.get("email_corporativo")
            or colaborador.get("email_pessoal")
            or colaborador.get("username")
            or ""
        ).strip().lower()
        is_owner_account = colaborador_email in allowed_emails

        if account_type == "personal" and not is_premium and not is_owner_account:
            raise HTTPException(status_code=403, detail="Recurso exclusivo do Plano Premium.")

        # Não bloqueia a conta do proprietário por trial expirado.
        if account_type == "personal" and not is_owner_account and premium_expires_at:
            try:
