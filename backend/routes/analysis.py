...
async def create_analysis(input_data: BiometricInput, request: Request):
    try:
        ...
        return AnalysisResponse(
            id=analysis.id, v_score=analysis.v_score,
...
async def get_history(request: Request, limit: int = 30):
    try:
        ...
        return [
            AnalysisResponse(
                id=a["id"], v_score=a["v_score"],
                area_afetada=a["area_afetada"], status_visual=a["status_visual"],
                tag_rapida=a["tag_rapida"], causa_provavel=a["causa_provavel"],
                nudge_acao=a["nudge_acao"], timestamp=a["timestamp"],
